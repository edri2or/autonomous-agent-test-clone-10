...
5. **Read the `current-focus` issue (mandatory orientation).** Use `mcp__github__list_issues` with `labels: ["current-focus"]` and `state: "open"` (or `gh issue list --label current-focus --state open --json number,title,body --limit 1`) on **this clone's repo** (each clone has its own current-focus issue tracking its local activation/runtime work). Treat its **Next Concrete Step** as your default task absent operator override. If zero issues are returned → halt and ask the operator which work item to pick up (do NOT invent one). Before session end, if state changed, update the issue body via `mcp__github__issue_write` with method `update`, bumping the **Updated By** line.
