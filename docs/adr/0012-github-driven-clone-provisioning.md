...
- A PAT (`gh-admin-token`) must be stored once globally in `or-infra-templet-admin` Secret Manager. Preferred form: fine-grained token scoped to the `edri2or` org with `repo + workflow + admin:org`.
