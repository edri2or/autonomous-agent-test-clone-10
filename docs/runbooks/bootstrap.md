...
1. **Set up GitHub App registration inputs.** Set GitHub Variables `GITHUB_ORG` (the org/user that will own the App; usually matches the clone repo owner) and `APP_NAME` (the App's display name) on the clone repo. Required because `bootstrap.yml:512` gates Phase 4's `github-app-registration` job on `vars.GITHUB_ORG != '' && vars.APP_NAME != ''`. Use `gh variable set --repo $CLONE_REPO`; no operator action.
...
| 3c | Paste `installation-id` to Variables | github.com browser | No API surface to read this without the App existing |
...
**Total irreducible operator clicks per clone: 3 (R-07) + 1 (R-04) + optional 1 (R-10 silo).** Acknowledged. Documented. No code workaround possible; tracked as vendor floors in CLAUDE.md §"Honest scope" and the risk register.
